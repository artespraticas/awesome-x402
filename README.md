...
* [Scrape Agent](https://scrapeagent.xyz) - Pay-per-use web scraping API for AI agents. Extract clean text, links, HTML, or metadata from any public URL for $0.01 USDC on Base. No API key, no subscription, no account. x402 V2 compliant with OpenAPI spec, `.well-known/x402` manifest, and `llms.txt` for agent autodiscovery. ([OpenAPI](https://scrapeagent.xyz/openapi.json))
