...
- Scrape Agent - Pay-per-use web scraping API for AI agents. Extract clean text, links, HTML, or metadata from any public URL for $0.01 USDC on Base. No API key, no subscription, no account required. x402 V2 compliant with OpenAPI spec and agent autodiscovery via `/.well-known/x402` and `/llms.txt`. ([OpenAPI](https://scrapeagent.xyz/openapi.json)) ([Website](https://scrapeagent.xyz))
